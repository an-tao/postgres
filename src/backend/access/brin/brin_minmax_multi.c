...
static int
merge_sort(FmgrInfo *cmp, Oid colloid, DatumRange *ranges_tmp, int nranges, MinmaxMultiRanges *ranges, Datum *values, int nvalues)
{
	int	range_idx;
	int	value_idx;
	int	out_idx ;
	...
	while (range_idx < ranges->nranges && value_idx < nvalues)
	{
		...
		compar = FunctionCall2Coll(cmp, colloid, ranges->values[2*range_idx], values[value_idx]);
...
static int
merge_sort_2(FmgrInfo *cmp, Oid colloid, Datum *values, int nvalues, Datum *values_a, int nvalues_a, Datum *values_b, int nvalues_b)
{
	int	a_idx;
	int	b_idx;
	int	out_idx ;
					   ...
					   MinmaxMultiRanges *ranges, Datum newval)
{
	...
	if (ranges->nvalues >= 0)
	{
		 ...
		 */
		if (ranges->nvalues - ranges->nsorted > 0)
			memcpy(values_qsort, &ranges->values[2*ranges->nranges + ranges->nsorted - 1], (ranges->nvalues - ranges->nsorted) * sizeof(Datum));

		values_qsort[nvalues_qsort-1] = newval;

		/* the actual sort of all the values */
		if (nvalues_qsort > 1)
			qsort_arg(values_qsort, nvalues_qsort, sizeof(Datum), compare_values, (void *) &cxt);

		/* now do the merge sort of ranges->values and values_qsort */
		merge_sort_2(cxt.cmpFn, colloid, values, nvalues, ranges->values + 2*ranges->nranges, ranges->nsorted, values_qsort, nvalues_qsort);
